"""
A geometric object is represented as a mesh. One can compose such meshes to
new entities, like a cell and a number of flagella.
The system class instantiates objects that can be both.
"""

from numpy import dot, cross, concatenate, zeros, diag
import numpy as np

def transformedShape(system,
                     translation=(0, 0, 0),
                     rotation=diag([1, 1, 1]),
                     velocity=(0, 0, 0),
                     angular=(0, 0, 0)):
...
class Triangulation(object):
    ...
    def center_of(self, id):
        ...
        coord_sum = np.sum(self.coordinates[i_a:i_e], axis=0)
...
if __name__ == '__main__':
    ...
    import numpy as np
    t1 = Triangulation('jou',
                       np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0]]),
                       np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0]]),
                       np.array([[0, 1, 2]]))
    print(t1.triangulation)
    t2 = Triangulation('nou',
                       np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0]]),
                       np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0]]),
                       np.array([[0, 1, 2]]))
    print(t2.triangulation)
